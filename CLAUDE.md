...
`ProviderClient` (`base.py`) is deliberately **blocking and loop-free** — no `max_turns`, because the runtime owns the loop. Three native providers (OpenAI, Anthropic Messages, Gemini) plus nine OpenAI-compatible vendors built from a shared factory, two resellers, and Ollama. **Compat-vendor keys resolve only from that vendor's own profile/env** — never falling back to the OpenAI key, so a configured OpenAI key is never sent to a third-party endpoint.
